...
-- Función auxiliar: obtenemos variables que derivan a un terminal
...
-- Función auxiliar: obtenemos variables que derivan de dos variables
...
BEGIN
    -- Obtenemos el string de entrada
    ...
    -- Fila 1: llenamos la diagonal (X_ii)
    ...
    ELSE
        FOR row_index IN 1..(string_length - fila_number + 1) LOOP
            ...
            -- Insertamos o actualizamos la celda (row_index, col_index)
            INSERT INTO matriz_cyk (i, j, x)
...
BEGIN
    ...
    -- Obtenemos el símbolo inicial de la gramática
    SELECT parte_izq INTO grammar_start_symbol
    FROM GLC_en_FNC
    WHERE start = true
    LIMIT 1;
    
    -- Verificamos si el símbolo inicial está en X[1, string_length]
    SELECT x INTO final_cell_vars
    FROM matriz_cyk
    WHERE i = 1 AND j = string_length;
    
    -- Verificamos si el símbolo inicial está en el conjunto final
